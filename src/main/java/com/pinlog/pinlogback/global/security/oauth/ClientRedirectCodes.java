 ...
 * 고쳐도 컴파일이 통과한다.
 */
final class ClientRedirectCodes {

	/** 로그인 왕복 실패. 사유를 그대로 노출하지 않고 고정 코드 하나만 준다. */
	static final String OAUTH_FAILED = "OAUTH_FAILED";

	/** 사용자가 공급자 화면에서 취소했다({@code access_denied}). */
	static final String WITHDRAWAL_CANCELLED = "WITHDRAWAL_CANCELLED";

	/** 해제 호출이 실패했거나, 해제는 됐는데 삭제가 실패했다. */
	static final String WITHDRAWAL_FAILED = "WITHDRAWAL_FAILED";

	/** 해제 대상을 판정하지 못했다 — 담당 클라이언트가 없는 공급자. */
	static final String WITHDRAWAL_UNLINK_FAILED = "WITHDRAWAL_UNLINK_FAILED";

	/** 인증된 공급자 계정이 탈퇴를 요청한 회원의 것이 아니다(BD-48 §⑤). */
	static final String WITHDRAWAL_ACCOUNT_MISMATCH = "WITHDRAWAL_ACCOUNT_MISMATCH";

	/** 탈퇴가 확정됐다. 오류가 아니므로 {@code ?error=}가 아닌 자기 파라미터로 나간다. */
	static final String WITHDRAWAL_COMPLETED_PARAMETER = "withdrawal";
	static final String WITHDRAWAL_COMPLETED = "completed";
