...
기능 하나를 시작해서 병합하기까지의 전체 순서는 [개발 워크플로우](docs/development/workflow.md)에, 리뷰·병합 기준은 [코드 리뷰 가이드](docs/development/code-review.md)에 있습니다. 세부 기준은 [패키지 구조 규약](docs/development/package-structure.md), [API 규약](docs/development/api-conventions.md), [API 문서화](docs/development/api-documentation.md), [에러 처리 규약](docs/development/error-handling.md), [로깅 규약](docs/development/logging.md), [설정·프로파일 규약](docs/development/configuration.md), [데이터베이스 규약](docs/development/database-conventions.md), [테스트 규약](docs/development/testing-conventions.md)을 따릅니다.
