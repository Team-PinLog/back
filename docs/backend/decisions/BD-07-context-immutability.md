...
- **정렬·표시** — `created_at`이 새 행 기준으로 갱신된다(M2 A안, 백엔드 주도). 목록에서 수정한 맥락이 최신으로 올라오는 것은 **의도된 동작이며 버그가 아니다.** *(2026-07-27 [BD-25](BD-25-context-origin-created-at.md)로 대체 — `origin_created_at`이 최초 작성 시각을 승계하고 정렬·표시 기준이 된다)*
...
- 최초 작성 시각 보존이나 수정 계보 추적 요구가 생기면 → `origin_context_id` 도입을 재검토한다. *(2026-07-27 발동 — 프론트의 최초 작성 시각 보존·오래된순 정렬 요구. [BD-25](BD-25-context-origin-created-at.md)에서 `origin_context_id` 대신 `origin_created_at`을 채택)*
