...
3. **부하 시나리오의 지도 호출을 사용자 경로와 맞추기** — `load-read.js`가
   ...
   **다만 접근 경로는 같다** — 측정해 보니 bbox 유무가 계획 모양을 바꾸지 않고
   ...
   (2026-08-04 추가 확인: 아래 「이후 코드 변경」의 [3a]/[3b]/[3c] 측정으로 **접근 경로가 세 갈래
   모두 같다**는 것이 확정됐다. 그러므로 이 후보는 순전히 "응답 크기 과대 계상" 문제이며
   계획 관측의 정확성과는 무관하다. 우선도를 더 낮춰도 된다.)
...
- `findMarkers`·`findMarkersWithinBounds` 양쪽 WHERE에
  `(:keyword is null or lower(p.name) like :keyword escape '!' or lower(p.address) like :keyword escape '!')`가
  **항상** 붙는다. keyword가 null이어도 단락 평가가 계획에 남는다.
- keyword 패턴은 양끝 와일드카드(`%…%`)라 어떤 인덱스도 이 LIKE를 덮지 못한다.
- 이름 정렬은 SQL이 아니라 자바에서 한다(collation 의존성 회피). 계획에 정렬 노드가 없는 것이 정상이다.

`loadtest/sql/explain-matrix.sql`의 [3] 블록을 바뀐 코드와 동기화하고 세 갈래로 나눠 다시 쟀다
(천만 건 볼륨, member 2792, 각 697행 후보):

| 갈래 | 계획 | 반환 | 시간 |
| --- | --- | --- | --- |
| [3a] bbox 없음 · keyword 없음 | Nested Loop + `place_pkey` | 697행 | 12.16 ms |
| [3b] bbox 있음 · keyword 없음 | Nested Loop + `place_pkey` | 397행 | 1.21 ms |
| [3c] bbox 있음 · keyword 있음 | Nested Loop + `place_pkey` | 7행 | 1.56 ms |

**접근 경로는 세 갈래 모두 동일하다.** 회원 record를 `uq_record_active`로 먼저 좁힌 뒤
place를 PK로 하나씩 확인하며 bbox·LIKE를 필터로 적용한다. LIKE가 인덱스를 쓰지 못하는 것은
사실이지만 697회 루프 안에서만 평가되므로 규모 문제가 되지 않는다 — **판정 4의 결론
(표 크기에 비례하지 않고 회원 데이터에만 비례한다)은 변경 후에도 유지된다.**
(이 표는 [3b]·[3c]가 [3a]의 워밍 이득을 받은 같은 실행 안의 값이라 갈래 간 절대값 비교에는
쓰지 않는다. 계획 모양 확인이 목적이다.)
