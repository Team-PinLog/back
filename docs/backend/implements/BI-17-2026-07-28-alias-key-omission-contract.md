...
- **상태**: ✅ 완료
- **날짜**: 2026-07-28
- **관련**: S15P11A705-116, [docs#20](https://github.com/Team-PinLog/docs/pull/20)
...
동작이 이미 그렇게 되어 있으므로 테스트는 처음부터 통과한다. **그래서 통과만으로는 이 테스트가 무언가를 고정한다는 증거가 되지 않는다.** 기각한 대안을 실제로 넣어 보고 잡히는지 확인했다(`Cursor.decode`를 뮤테이션 검사로 고정한 S15P11A705-42와 같은 방식).
