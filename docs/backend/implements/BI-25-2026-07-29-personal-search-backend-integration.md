...
- **~~`ai` 레포 쪽 §7.1 대칭이 아직 없다~~ → 해소됐다(같은 날).** 작성 시점 관측은 "`config.py`에
  기본값이 없고 P45 파일도 없다"였는데 **낡은 관측이었다.** `ai#36`(`463cad5`)이 그 사이에 병합되어
  임베딩 설정 넷 전부에 기본값이 붙었고 P45와 `model-profile.md` §2.1 개정이 함께 들어왔다.
  계약(`docs#27` 07:52)과 구현(`ai#36` 07:59)은 22분 간격의 같은 흐름이었다.

  확인한 김에 **두 리터럴을 대조했다 — 바이트 단위로 같다.** `application.yml`의
  `openai-text-embedding-3-small-1536-cosine-v1`과 `ai` `origin/main` `config.py:53`이 일치한다.
  이것이 어긋나면 런타임 대조가 모든 검색을 422로 만들므로, "두 정본이 코드에 있다"보다
  **"두 값이 같다"**가 실제로 지켜야 할 명제다. 다만 그 대조는 지금 사람이 한 것이고 자동화된
  장치는 없다 — 위 "검증하지 않은 것"의 배포 스모크가 그 자리를 메워야 한다.
