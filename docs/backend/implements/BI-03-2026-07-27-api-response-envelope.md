...
- `DeploymentContractTests`·`OpenApiDocsTests`(회귀 감시, 기존 테스트를 수정하지 않고 그대로 통과) — `/api/core/actuator/health`가 봉투 없이 `{"status":"UP"}` 그대로, `/api/core/actuator/prometheus` 200, 미매핑 URL(`/api/core/not-found`, `/actuator/health`)이 404, `/api/core/v3/api-docs`가 200 + `"openapi"` 키를 포함한 유효한 문서.
