...
임계값 0.35는 back이 정하지 않았다. 인계 문서 스스로 "다른 용도(키워드 재정렬 floor)에서 가져온 값이라 이 용도로는 검증된 적이 없다"고 밝혔고, ai 파트가 별도로 오프라인 재측정해(Jira 작업) 같은 값을 다시 채택했다. back은 그 결과를 설정 기본값으로 가져오기만 했다.
