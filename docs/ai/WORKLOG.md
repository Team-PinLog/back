...
시간순 작업 로그입니다. 작업 기록이 유형별 폴더(spec/proposals/implements/troubleshooting)로 분산되어 있어, 한 작업의 전체 흐름을 추적하려면 여러 폴더를 오가야 합니다. 이 파일은 그 비용을 줄이기 위한 시간축 색인입니다. **이후 작업마다 항목 하나를 추가합니다. 항목은 몇 문장 이내로 간결하게 쓰고, 상세는 implements 리포트에 둡니다.**
