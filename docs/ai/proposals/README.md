...
이 폴더는 **back 레포의 결정**을 담는다. 어느 파트가 주도했든 back 아티팩트(`build.gradle`·`application.yml`·`db/migration`)에 영향을 주는 결정을 `P##` 번호로 기록한다. `P##`는 전수 인벤토리의 제안 번호와 일치한다.
...
| [P24](P24-flyway-schemas-unspecified.md) | `flyway.schemas` 미지정. 이력 테이블을 public에 배치 | Accepted | 백엔드 |
| [P42](P42-feed-mvp-without-place-metadata.md) | MVP Feed에서 Place category·region 제외 | Accepted | AI |
| [P46](P46-feed-keyword-display-order.md) | Feed 응답 Keyword의 동점 규칙(축 내 순위, 다음 `preset.id`) | Accepted | AI |
...
| P24 | `flyway.schemas` 미지정으로 이력 테이블을 public에 배치 | Accepted | [P24](P24-flyway-schemas-unspecified.md) |
...
| P46 | Feed 응답 `keywords` 동점 규칙(축 내 순위, 다음 `preset.id`). 개수 3·빈도 내림차순은 프론트 구두 합의 | Accepted | [P46](P46-feed-keyword-display-order.md) |

## 미결 (백엔드 관련)

| M | 쟁점 | 상태 |
|---|---|---|
| M2 | Context 목록 `created_at` 시각 기준(최초 작성 vs 현재)과 `origin_context_id` 계보 필드 여부. **core `V2~` migration을 막고 있는 쟁점** | 미결 |
