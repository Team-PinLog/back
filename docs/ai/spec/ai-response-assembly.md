...
- **`context_ai_state` 행이 없으면 `COMPLETED`입니다.** 백엔드가 Context 생성과 같은 트랜잭션에서 `PENDING` 행을 넣으므로(`ContextAiStateRepository`) 정상 경로에서는 나오지 않는 상태이고, 이것을 `PROCESSING`으로 접으면 **영영 오지 않는 것에 "분석 중"을 띄우게 됩니다** — 이 개정이 없애려는 바로 그 증상입니다.
...
**뒤집지 않은 것도 적어 둡니다.** 이전 판단이 든 비용 근거 — *"상태 필드를 넣으면 모든 클라이언트가 3상태를 분기해야 한다"* — 는 **여전히 유효합니다.** 그래서 `null` 대신 빈 배열을 유지하고(`5`의 첫 규칙), 노출을 검색 응답 하나로 좁히고, 필드를 무시하면 개정 전과 같도록 설계했습니다. 분기가 **필요한** 클라이언트는 `PROCESSING` 하나만 보면 됩니다.
