...
실측에서 **4개로 자를 필요가 있는 Collection 6건 전부가 4위와 5위의 빈도가 같았습니다(6/6).** 빈도 내림차순은 경계에서 한 건도 가르지 못합니다 — Collection이 Record 1~5건 규모이고 Context 하나가 Keyword를 평균 2개 받아 같은 Keyword가 겹칠 일이 드물기 때문입니다. **잘리는 자리는 전부 동점 규칙이 정합니다.**
