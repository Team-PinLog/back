...
정렬 키는 `(빈도 DESC, 축 내 순위 ASC, preset id ASC)`이며 **출처가 다릅니다** — 개수 4와 빈도 내림차순은 프론트와의 구두 합의(2026-08-03), 동점 규칙은 back AI 파트의 판단입니다.

| # | 항목 | 기대 |
|---|---|---|
| KW1 | Keyword가 5개 이상인 Collection | 응답에 **4개**만. 상한이 상수이며 설정으로 바뀌지 않음 |
| KW2 | 빈도가 갈리는 Collection | 빈도 높은 것이 앞. 축이 달라도 빈도가 낮으면 뒤 |
| KW3 | 빈도가 전부 같고 축이 4개 | 상위 4개가 **서로 다른 축**. 실측상 자를 일이 생기는 Collection 전부가 경계 동점이라 이 경로가 기본 |
...
**KW2와 KW3을 함께 봐야 축의 자리가 드러납니다.** 축은 **같은 빈도 안에서만** 일하고 빈도를 뒤집지 않습니다(KW2). 그런데 실측에서 4개로 자를 필요가 있는 Collection 6건 **전부**가 4위와 5위의 빈도가 같아, 잘리는 자리는 사실상 전부 축이 정합니다(KW3). 한쪽만 검증하면 구현이 어느 쪽으로도 흘러갑니다 — 특히 **축을 전역 1순위로 올리는 구현은 빈도가 전부 같은 데이터에서 결과가 똑같아** KW3만으로는 걸리지 않습니다.
