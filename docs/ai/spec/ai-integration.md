...
`internal-secret`은 저장소가 public이므로 기본값을 비워 둡니다. 값이 없으면 운영 프로파일은 **기동에 실패하고** 그 외에는 경고만 남깁니다. 시크릿이 없을 때의 실패 모드가 무음이기 때문입니다(`process`는 모든 실패를 삼킵니다). 그래서 운영에서는 기동을 막는 편이 안전합니다. 헤더 이름은 §7의 `X-Internal-Secret`이며 원본은 `ai` 레포 `app/core/security.py`입니다.
